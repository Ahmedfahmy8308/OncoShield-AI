          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-surface rounded-lg p-3">
                ...
                <p className="font-h3 text-error text-[24px] font-bold">{stats ? stats['0_malignant'] : 212}</p>
              </div>
              <div className="bg-surface rounded-lg p-3">
                <p className="font-body-sm text-outline mb-1 text-[14px]">Benign (B)</p>
                <p className="font-h3 text-secondary text-[24px] font-bold">{stats ? stats['1_benign'] : 357}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="gap-gutter grid grid-cols-1 items-start lg:grid-cols-12">
            ...
            <div className="space-y-6">
              ...
              <div className="flex gap-4 mb-4">
                <button 
                  ...
                  className="bg-error/10 hover:bg-error/20 text-error px-4 py-2 rounded-lg font-bold text-[13px] transition-all">
                  Load Malignant Case
                </button>
                <button 
                  onClick={() => setFeatures(BENIGN_PROFILE)}
                  className="bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-600 px-4 py-2 rounded-lg font-bold text-[13px] transition-all">
                  Load Benign Case
                </button>
              </div>
              <div className="grid grid-cols-2 gap-x-4 gap-y-4 max-h-[420px] overflow-y-auto pr-2" style={{ scrollbarWidth: 'thin' }}>
                {Object.keys(MALIGNANT_PROFILE).map((key) => (
                  <div key={key} className="space-y-1.5">
                    <label className="font-body-sm text-on-surface-variant text-[12px] font-semibold capitalize flex justify-between">
                      <span className="truncate pr-1">{key.replace('mean ', 'Mean ').replace(' error', ' Error').replace('worst ', 'Worst ')}</span>
                    </label>
                    <input
                      className="bg-surface-container-lowest border-outline-variant/50 focus:border-primary focus:ring-primary/20 w-full rounded-lg border px-3 py-2 text-[13px] transition-all focus:outline-none focus:ring-2"
                      type="number"
                      step="any"
              ...
              <div className="bg-surface relative flex flex-col items-center rounded-xl border border-slate-100 p-6">
                <div className="border-outline-variant/30 relative grid aspect-square w-full max-w-[240px] grid-cols-2 grid-rows-2 overflow-hidden rounded-full border-2">
                  <button 
                    ...
                    <span className={`absolute top-4 left-4 text-[10px] font-bold ${quadrant === 'UIQ' ? 'text-primary' : 'text-outline group-hover:text-primary'}`}>
                      UIQ
                    </span>
                    {quadrant === 'UIQ' && <span className="bg-error absolute top-1/2 left-1/2 h-3 w-3 -translate-x-1/2 -translate-y-1/2 animate-pulse rounded-full shadow-[0_0_8px_rgba(186,26,26,0.6)]"></span>}
                  </button>
                  <button 
                    ...
                    <span className={`absolute top-4 right-4 text-[10px] font-bold ${quadrant === 'UOQ' ? 'text-primary' : 'text-outline group-hover:text-primary'}`}>
                      UOQ
                    </span>
                    {quadrant === 'UOQ' && <span className="bg-error absolute top-1/2 left-1/2 h-3 w-3 -translate-x-1/2 -translate-y-1/2 animate-pulse rounded-full shadow-[0_0_8px_rgba(186,26,26,0.6)]"></span>}
                  </button>
                  <button 
                    ...
                    <span className={`absolute bottom-4 left-4 text-[10px] font-bold ${quadrant === 'LIQ' ? 'text-primary' : 'text-outline group-hover:text-primary'}`}>
                      LIQ
                    </span>
                    {quadrant === 'LIQ' && <span className="bg-error absolute top-1/2 left-1/2 h-3 w-3 -translate-x-1/2 -translate-y-1/2 animate-pulse rounded-full shadow-[0_0_8px_rgba(186,26,26,0.6)]"></span>}
                  </button>
                  <button 
                    ...
                    <span className={`absolute right-4 bottom-4 text-[10px] font-bold ${quadrant === 'LOQ' ? 'text-primary' : 'text-outline group-hover:text-primary'}`}>
                      LOQ
                    </span>
                    {quadrant === 'LOQ' && <span className="bg-error absolute top-1/2 left-1/2 h-3 w-3 -translate-x-1/2 -translate-y-1/2 animate-pulse rounded-full shadow-[0_0_8px_rgba(186,26,26,0.6)]"></span>}
                  </button>
        ...
        <div className="space-y-gutter lg:col-span-4">
          <div className="overflow-hidden rounded-xl border border-slate-100 bg-white shadow-[0_4px_6px_-1px_rgba(2,132,199,0.05)]">
            {prediction ? (
              <div className={`${prediction === 'MALIGNANT' ? 'bg-error-container/20 border-error/10' : 'bg-emerald-50 border-emerald-100'} flex flex-col items-center border-b p-8 text-center`}>
                <span
                ...
                <h4 className={`font-label-caps ${prediction === 'MALIGNANT' ? 'text-error' : 'text-emerald-700'} mb-1 text-[12px] font-bold tracking-[0.05em] tracking-widest`}>
                  DIAGNOSTIC RESULT
                </h4>
                <p className={`font-h2 ${prediction === 'MALIGNANT' ? 'text-error' : 'text-emerald-600'} mb-4 text-[32px] font-black`}>{prediction}</p>
                <div className={`${prediction === 'MALIGNANT' ? 'bg-error/10' : 'bg-emerald-200'} mb-4 h-1.5 w-full rounded-full`}>
                  <div className={`${prediction === 'MALIGNANT' ? 'bg-error' : 'bg-emerald-500'} h-full rounded-full`} style={{ width: `${(confidence || 0) * 100}%` }}></div>
                </div>
                <div className="flex flex-col items-center">
                  <span className={`font-h1 ${prediction === 'MALIGNANT' ? 'text-on-error-container' : 'text-emerald-800'} text-[40px] font-bold`}>{((confidence || 0) * 100).toFixed(1)}%</span>
                  <span className={`font-body-sm ${prediction === 'MALIGNANT' ? 'text-on-error-container/70' : 'text-emerald-700/70'} text-[14px]`}>
                    Confidence Score
                  </span>
                </div>
              </div>
            ) : (
               <div className="flex h-64 flex-col items-center justify-center p-8 text-center bg-surface-container-lowest">
                  <span className="material-symbols-outlined text-outline-variant mb-4 text-5xl">biotech</span>
                  <p className="text-on-surface-variant font-semibold">Waiting for input...</p>
                  <p className="text-outline text-sm mt-2">Adjust morphological parameters and run analysis to see live predictions.</p>
               </div>
            )}
          </div>
            ...
            <div className="rounded-xl border border-slate-100 bg-white p-6 shadow-[0_4px_6px_-1px_rgba(2,132,199,0.05)]">
              ...
              <div className="border-outline-variant/30 group relative overflow-hidden rounded-lg border">
                ...
                <div
                  ...
                  style={{
                    background: prediction === 'MALIGNANT' ? 
                ...
                ></div>
                <div className={`${prediction === 'MALIGNANT' ? 'border-error/40' : 'border-emerald-500/40'} pointer-events-none absolute inset-0 rounded-lg border-2`}></div>
                ...
                {prediction === 'MALIGNANT' ? (
                  <>
                    <li className="flex gap-3">
                      ...
                      <p className="font-body-sm text-on-surface-variant text-[14px] leading-tight">Priority oncological consultation required within 48 hours.</p>
                    </li>
                    <li className="flex gap-3">
                      <span className="material-symbols-outlined text-secondary shrink-0 text-[20px]">check_circle</span>
                      <p className="font-body-sm text-on-surface-variant text-[14px] leading-tight">Order core needle biopsy (CNB) for histopathological validation.</p>
                    </li>
                    <li className="flex gap-3">
                      <span className="material-symbols-outlined text-secondary shrink-0 text-[20px]">check_circle</span>
                      <p className="font-body-sm text-on-surface-variant text-[14px] leading-tight">Flag record for Multidisciplinary Tumor Board (MTB) review.</p>
                    </li>
                  </>
                ) : (
                  <>
                    <li className="flex gap-3">
                      <span className="material-symbols-outlined text-emerald-500 shrink-0 text-[20px]">check_circle</span>
                      <p className="font-body-sm text-on-surface-variant text-[14px] leading-tight">Maintain standard annual screening protocol.</p>
                    </li>
                    <li className="flex gap-3">
                      <span className="material-symbols-outlined text-emerald-500 shrink-0 text-[20px]">check_circle</span>
                      <p className="font-body-sm text-on-surface-variant text-[14px] leading-tight">Monitor for interval changes in structural symmetry.</p>
                    </li>
                  </>
                )}
              </ul>
